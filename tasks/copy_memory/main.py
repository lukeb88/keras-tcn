...
class PrintSomeValues(keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs={}):
        print('x_test[0:1] = {}.'.format(x_test[0:1].flatten()))
        print('y_test[0:1] = {}.'.format(y_test[0:1].flatten()))
        print('p.shape = {}.'.format(self.model.predict(x_test[0:1]).shape))
        print('p(x_test[0:1]) = {}.'.format(self.model.predict(x_test[0:1]).argmax(axis=2).flatten()))
